...
from ..utils import gen_expander
...
class MaskedLinear(Module):
    r"""Applies a linear transformation to the incoming data: :math:`y = Ax + b`
    with an enforcing condition that some of the elements in A be zero. This
    effectively blocks some of the connections in the neural network.
    Args:
        in_features: size of each input sample
        out_features: size of each output sample
        degree: the degree of each neuron in the previosu layer
        bias: If set to False, the layer will not learn an additive bias. Default: True
    Shape:
        - Input: :math:`(N, in\_features)`
        - Output: :math:`(N, out\_features)`
    ...
    def __init__(self, in_features, out_features, degree, bias=True):
        ...
        assert (type(in_features)==int), "in_features should be int"
        assert (type(out_features)==int), "out_features should be int"
        assert (type(degree)==int), "degree should be int"
        self.in_features = in_features
        self.out_features = out_features
        self.degree = degree
        self.weight = Parameter(torch.Tensor(out_features, in_features))
        self.mask = torch.from_numpy(gen_expander(self.in_features, self.out_features, self.degree)).float()
        if bias:
            self.bias = Parameter(torch.Tensor(out_features))
        else:
            self.register_parameter('bias', None)
    ...
    def forward(self, input):
        self.weight.data = torch.mul(self.weight.data, self.mask)
        if self.bias is None:
